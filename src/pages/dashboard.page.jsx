...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const getMonthlyInflow = async () => {
      try {
        setLoading(true)
        const response = await axios.get(API.GET_MONTHLY_INFLOW)
        setInflow(response.data.data)
      } catch (err) {
        errorNotify(err.response.data.message)
        setLoading(false)
      }
    ...
    const getMonthlyOutflow = async () => {
      try {
        const response = await axios.get(API.GET_MONTHLY_OUTFLOW)
  ...
  return (
    <Layout>
      <CustomContainer otherStyles="lg:grid lg:grid-cols-4 mt-6 lg:divide-x">
        <StatCard
          title="Cars On Rent"
          img={require('../assets/images/car-rent.png')}
          stat={20}
        />
        <StatCard
          title="Total Amount"
        ...
        <StatCard
          ...
          img={require('../assets/icons/car-rental.png')}
          stat={totalCars}
          loading={loading}
        />
        <StatCard
      ...
      <CustomContainer otherStyles="mt-10 p-4 w-full flex flex-col xl:flex-row space-x-2">
        <div className="w-full">
          ...
          <div className="mt-4 flex space-x-3">
            <CustomTable
              columns={rentedVehicleInfoTableColumns}
              data={rentedVehicleInfoData}
              perPage={5}
            />
          </div>
        </div>
