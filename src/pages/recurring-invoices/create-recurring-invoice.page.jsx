...
export default function CreateRecurringInvoice() {
    ...
    const [nextInvoice, setNextInvoice] = useState(Date.now())
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            console.log(customerName, customerEmail, invoiceDate, nextInvoice)
            if (!customerName) {
                setLoading(false)
            ...
            if (!nextInvoice) {
                setLoading(false)
                return errorNotify('Next invoice date is required')
            }
            if (items.length < 1) {
                setLoading(false)
            ...
            await axios.post(API.CREATE_RECURRING_INVOICE, {
                customer: customerId,
                nextInvoice: selectedValue,
                customerEmail,
                customerName,
                invoiceDate,
                total,
            ...
            successNotify('Recurring Invoice Created.')
            setCustomerEmail('')
            setCustomerName('')
            setInvoiceDate('')
            setNextInvoice('')
        ...
        <CustomContainer otherStyles="p-6 flex flex-col xl:flex-row mt-8">
            <div>
                <div className="flex items-center space-x-3">
                    <h1 className="font-bold text-xl">Create Recurring Invoice</h1>
                </div>
                    ...
                    <div className="w-8/12">
                        <div className="flex w-full flex-wrap justify-end">
                            ...
                            <ListDropdown
                                ...
                                list={['Within 7 Days', 'Within 14 Days', 'Within 30 days', 'Within 45 days']}
                                selectedValue={selectedValue}
                                setSelectedValue={setSelectedValue} showSearch={false}/>
                        </div>
                    </div>
                </form>
                <div className="mt-4">
                      ...
                      X
                          ...
                          {customerName ? customerName : 'Not Selected'}
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody>
                                    {items.map((item) => (<tr className="border-b-2 border-slate-200">
