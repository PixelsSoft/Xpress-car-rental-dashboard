...
export default function Menu() {
  ...
  const menus = [
    ...
    // { id: 6, title: 'Transactions', icon: require('../assets/icons/card-tick.png'), to: '/transactions' },
    {
      id: 7,
      title: 'Expenses',
      icon: require('../assets/icons/expense-icon.png'),
      to: '/expenses',
    },
    {
      id: 8,
      title: 'Vendors',
      icon: require('../assets/icons/vendor-icon.png'),
      to: '/vendors',
    },
  ]
  ...
  return (
    <>
      <div
        className={`shadow-md ${
          openMenu && 'hidden'
        } w-fit h-fit fixed p-3 m-4 rounded-full bg-[#FEBD20] md:hidden cursor-pointer flex z-50`}
      >
      ...
      <div
        className={`w-[320px] ${
          !openMenu && 'hidden'
        } md:flex h-screen fixed md:left-2 md:top-0 md:p-4 z-10`}
      >
        ...
        <div
          ...
          } md:rounded-2xl h-full md:flex p-3 flex-col items-center justify-between`}
        >
                ...
                >
                  ...
                  {menu.subMenu && (
                    <MdKeyboardArrowDown
                      ...
                      onClick={() => {
                        if (menu.title === 'User')
                          return setUserSubMenuOpen(!userSubMenuOpen)
                        if (menu.title === 'Sales & Payments')
                          return setSalesAndPaymentsSubMenu(
                            !salesAndPaymentsSubMenu,
                          )
                      }}
                    />
                  )}
                    ...
                    {menu.title === 'User' &&
                      menu.subMenuItems.map((subMenuItem, index) => (
                        <li
                          ...
                          className="text-sm font-semibold border-l-2 border-gray-300 text-white flex items-center gap-x-4 cursor-pointer p-2 px-5"
                        >
                          {subMenuItem.title}
                        </li>
                      ))}
